...
this.each(function(){ 
	numkeyboardcount++;
    //添加键盘    
    if(numkeyboardcount>1){
		return false;
	}
    $("body").append("<ul id='keyboard_number'>"+
    "<li>1</li>"+
    "<li>2</li>"+
    "<li>3</li>"+
    "<li>4</li>"+
    ...
    "<li>←</li>"+
//  "<li>关闭</li>"+
    "<div id='keyboard_number_left'></div>"+
    "<div id='keyboard_number_right'></div>"+
    "</ul>");     
    
    var inputele = $(this);
    var keyboard =$("#keyboard_number");
...
//    if(options.keyboardRadixChange){//设置是否允许缩放
//        BmouseDrag();
//        BtouchDrag();
//    }
    /*		双击定位
    ...
    */
    ...
    keys.unbind();//移除所有的绑定,防止重复绑定事件。
    keys.click(function(event){
//    	dbclick++;
//    	if(dbclick%2 == 0){//莫名会执行两次，所以做个判断
		    var keyele = $(this);
		    var keytext = keyele.text();
		    var evebool = true; 
		    activeinputele = activeinputele?activeinputele:foucsInput;
		    if(keytext==="CapsLock"){
...
//	    }
    ...
    inputele.focus(function(event){
    	...
    	foucsInput = activeinputele;
	    if($(activeinputele[0]).attr("readonly")){//如果当前文本框 为只读，则不显示 键盘
    		return;
    	}
...
//    	debugger
    	if(activeinputele){
    		...
    		return;
    		var inputtext = foucsInput.val();
    		inputtext = inputtext+key;        
    		foucsInput.val(inputtext);
    		foucsInput[0].focus();
    	}
    }    
