            ...
            <div class="panel panel-info">
                ...
                <div class="panel-body">
                    <table id="brandOrderTable" class="table table-striped table-bordered table-hover" width="100%">
                        ...
                        <tbody>
                        <template v-if="brandAppraise.brandName != null">
                            <tr>
                                <td><strong>{{brandAppraise.brandName}}</strong></td>
                                <td>{{brandAppraise.brandNum}}</td>
                                <td>{{brandAppraise.appraiseRatio}}%</td>
                                <td>{{brandAppraise.allLevel}}</td>
                                <td>{{brandAppraise.service}}</td>
                                <td>{{brandAppraise.exhibit}}</td>
                                <td>{{brandAppraise.ambience}}</td>
                                <td>{{brandAppraise.conditions}}</td>
    ...
    var vueObj =  new Vue({
        ...
        methods:{
                    ...
                    columns : [
                        ...
                        },
                        {
                            ...
                            createdCell: function (td, tdData, rowData) {
                                var button = $("<a href='newAppraiseReport/shopReport?beginDate="+that.searchDate.beginDate+"&&endDate="+that.searchDate.endDate+"&&shopId="+tdData+"' class='btn green ajaxify '>查看详情</a>");
                                $(td).html(button);
                            }
                        }
                    ]
            ...
            searchInfo : function() {
                ...
                toastr.success("查询中...");
                try {
                    $.post("newAppraiseReport/brand_data", this.getDate(null), function (result) {
                        if (result.success) {
                            that.brandAppraise = result.data.brandAppraise[0];
