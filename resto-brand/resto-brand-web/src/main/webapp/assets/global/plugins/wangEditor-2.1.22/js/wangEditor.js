...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            resultText = JSON.parse(resultText);//转为JSON格式
            ...
            if (resultText.success) {
                ...
                img.onload = function () {
                    var html = '<img src="' + resultText.data + '" alt="' + originalName + '" style="max-width:100%;"/>';
                    editor.command(null, 'insertHtml', html);
                    E.log('已插入图片，地址 ' + resultText.data);
                    img = null;
                };
                ...
                img.src = resultText.data;
            } else {
                // 提示错误
                E.warn('上传失败：' , resultText);
                toastr.error("图片上传失败！");
            }
